...
def run_reward(reward_model_id: str, instructions: List[str], 
               output_dir: str,
               output_filename: str,
               num_instructions: int,
               num_annotations: int,
               west_of_n: bool,
               dmbr_result: pd.DataFrame) -> pd.DataFrame:
    ...
    for sample_id in tqdm(range(num_instructions)):
        ...
        if west_of_n:
            row = [input_instruction] + [ordered_responses[0], ordered_responses[-1]] + [ordered_rewards[0], ordered_rewards[-1]]            
        else:
    ...
    if (num_annotations == 2) or west_of_n:
        ...
        columns = ['prompt'] + ['pref_{}'.format(i) for i in range(num_annotations)] + ['pref_{}_r'.format(i) for i in range(num_annotations)]
