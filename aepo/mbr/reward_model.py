...
class RewardModel:
    ...
    def get_pairwise_reward(self, question: str, answer: str, compared_answer: str) -> float:
        pass
...
class Eurus(RewardModel):
    ...
    def __init__(self, reward_model_id):
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.tokenizer = AutoTokenizer.from_pretrained(reward_model_id)
        self.model = AutoModel.from_pretrained(reward_model_id, trust_remote_code=True).to(self.device)
        self.model.eval()
...
class PairLM(RewardModel):
    ...
    def __init__(self, reward_model_id):
        ...
        self.pairrm = DebertaV2PairRM.from_pretrained("llm-blender/PairRM-hf").to(self.device)
        self.pairrm.eval()
    ...
    def tokenize_pair(self, sources:List[str], candidate1s:List[str], candidate2s:List[str], source_max_length=1224, candidate_max_length=412):
        ids = []
        assert len(sources) == len(candidate1s) == len(candidate2s)
        max_length = source_max_length + 2 * candidate_max_length
        for i in range(len(sources)):
            source_ids = self.tokenizer.encode(self.source_prefix + sources[i], max_length=source_max_length, truncation=True)
...
class Starling(RewardModel):
    def __init__(self, reward_model_id):
        ...
        self.reward_model = GPTRewardModel("meta-llama/Llama-2-7b-chat-hf").to(self.device)
    ...
    def get_reward_(self, samples):
        ...
        for i in range(math.ceil(len(samples) / mbs)):
            rewards = self.reward_model(input_ids=input_ids[i * mbs : (i + 1) * mbs], attention_mask=attention_masks[i * mbs : (i + 1) * mbs])
            out.extend(rewards)
