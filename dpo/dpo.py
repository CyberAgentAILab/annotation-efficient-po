...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', help="dataset name", default='XXX/alpacafarm_dpo')
    parser.add_argument('--split', help="split name", default='kmbr_test')
    ...
    parser.add_argument('--username', help="username for hub", default="XXX")
    ...
    if len(hub_id) > 96:
        name = "{}_{}_{}_{}_{}_{}_{}".format(model_name.split('/')[1][:4],
                                                        args.dataset.split('/')[1][:4],
                                                        args.split,
                                                        args.beta,
                                                        args.lora_r,
                                                        args.lr,
    ...
    train_dataset = load_dataset(dataset, split='{}[:{}]'.format(args.split, train_size), 
                                 use_auth_token='XXX').shuffle(seed=42)
    ...
    dpo_trainer.model.push_to_hub(hub_id, revision=dtime, commit_message='auto commit from dpo.py', 
                                  private=True, use_auth_token='XXX')
